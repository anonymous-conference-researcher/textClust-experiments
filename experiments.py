...
def evaluate_faststream(listtuple_pred_true_text, ignoreMinusOne=False):
    preds =[]
    trues = []
...
for dataset in datasets:
    ...
    ## textclust
    start = time.time()
    re = text_eval.run(dataDir+dataset, datalen, gram =1)
    ...
    ## textclust
    start = time.time()
    re = text_eval.run(dataDir+dataset, datalen, gram =2)
    runtime = time.time()-start
    ...
    print("FASTSTREAM")
    print("----------------------------------------------------------------------------")
    currentAlgo = "faststream"
    ## faststream 
    start = time.time()
    re = faststream.run(dataDir+dataset, "FastStream_Result/")
    runtime = time.time()-start
    
    res = evaluate_faststream(re)
